...
void run(Date* date, Task** tasks, enum runType format) {
	...
	while ((ch = _getch()) != 27)
	{
		...
		else if ((char)ch == 'c' && format == day) on_copy(tasks);
		else if ((char)ch == 'e' && format == day) on_edit(tasks);
		else if ((char)ch == 'r' && format == day) on_remove(tasks);
