...
void render_options(RunType format)
{
...
void render_month(Date *date, Task **tasks)
{
	...
	// Redner header
	printf("\t%d\t\t\t%s\n\r\t", date->tm_year + 1900, get_month_name(date->tm_mon));
	for (int i = 0; i < 7; i++)
		printf("%s\t", week_day_names[i]);
	printf("\n\r\t");
	for (int i = 0; i < 52; i++)
		printf("-");
	printf("\n\r");
	...
	{
		// Selected date
		if (date->tm_mday == tmp.tm_mday)
			printf(WHITE_BACK "\t%d" RESET, tmp.tm_mday);
		else if (date_has_tasks(tmp, tasks))
			printf(YELLOW "\t%d" RESET, tmp.tm_mday);
		else
...
void render_day(Date *date, Task **tasks, int move_task)
{
	...
	printf("\t%d. %s %d\n\r", date->tm_mday, get_month_name(date->tm_mon), date->tm_year + 1900);
	printf("\n\r");
...
void render_week(Date *date, Task **tasks, int move_task)
{
	...
	for (int i = 0; i < 7; i++)
	{
		if (tmp.tm_mday == date->tm_mday)
			printf("\t%s " WHITE_BACK "%d" RESET, week_day_names[i], tmp.tm_mday);
		else
			printf("\t%s %d", week_day_names[i], tmp.tm_mday);
		printf(" ");
...
{
	if (format == day)
		render_day(date, tasks, move_task);
	else if (format == week)
		render_week(date, tasks, move_task);
	else if (format == month)
		render_month(date, tasks);
}